...
rm -rf ~{0}/.i2p""".format(project_dir)
  ...
  child.sendline("0")
  logging.debug("Sent 0")
  child.expect("Press 1 to continue, 2 to quit, 3 to redisplay",timeout=30)
  logging.debug("Continue question #1")
  child.sendline("1")
  logging.debug("Continue answer #1")
  child.expect("Press 1 to continue, 2 to quit, 3 to redisplay",timeout=30)
  logging.debug("Continue question #2")
  child.sendline("1")
  logging.debug("Continue answer #2")
  child.expect("Select the installation path:.*",timeout=30)
  logging.debug("Path question")
  child.sendline(location)
  logging.debug("Sent path answer {0}".format(location))
  child.expect("Press 1 to continue, 2 to quit, 3 to redisplay",timeout=30)
  logging.debug("Continue question #3")
  child.sendline("1")
  logging.debug("Continue answer #3")
  #child.expect("Enter O for OK, C to Cancel:",timeout=30)
  #logging.debug("Ask for OK or Cancel")
  #child.sendline("O")
  #logging.debug("Sent OK")
  child.expect("Press 1 to continue, 2 to quit, 3 to redisplay",timeout=30)
  logging.debug("Continue question #4")
  child.sendline("1")
  logging.debug("Continue answer #4")
  child.expect("Press 1 to continue, 2 to quit, 3 to redisplay",timeout=30)
  logging.debug("Continue question #5")
  child.sendline("1")
  logging.debug("Continue answer #5")
  child.expect(".*Console installation done.*",timeout=30)
