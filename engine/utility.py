...
def appendFileData(filename, prefix ,token, data):
  """
  Appends text to file. Text is appended with start prefix, formed with prefix+token+'begin'+'\n'+data+...'end\n'
  Useful to change config files. Token should have something meaningfull e.g. 'section for feature XXX', prefix should contain comment symbol at first.
  Do not apply to config files, which do not support comments!  
  """
  fileText=""
  dataToAppend="\n"+prefix+token+" begin\n"+data+"\n"+prefix+token+" end"
...
def removeFileData(filename,prefix, token):
  """
  Removes section from file, previously generated by 'appendFileData'. Must have same token and prefix, as used for appending.
  """
  fileText=""
  regexp=prefix+token+"begin.*?"+prefix+token+"\send\n"
