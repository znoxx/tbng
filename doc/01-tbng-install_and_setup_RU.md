...
Протестированная версия Java — 8. 
...
Скрипты автоустановки пакетов выполняются ощутимое время. В процессе будут удалены tor и privoxy, причём их конфигурация будет сохранена в резервную копию. Также будет подключено несколько дополнительных репозиториев для node.js.

##### OpenJDK и Debian Buster

В Debian Buster отсутствует пакет OpenJDK версии 8.

Если в результате установки возникла ошибка установки пакета OpenJDK, то нужно вручную изменить номер версии в файле **tbng/setup/apt/09-install**.
...
### Особенности Ubuntu 18.04 (Bionic Beaver) и других дистрибутивов, где используется systemd-resolved
...
Для проверки того, что в системе действительно используется dnsmasq, выполните команду (возможно, потребуется доустановить пакет, содержащий утилиту nslookup):

`nslookup github.com`

Её вывод должен содержать информацию о вашем DNS-сервере. Если это 127.0.0.1, то цель достигнута. Вот пример:
...
```
Если в ответе использован другой DNS-сервер (не 127.0.0.1#53), то нужна перенастройка сервиса разрешения имён. Обратитесь к документации на вашу ОС.
