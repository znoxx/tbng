...
```
Также рекомендуется в файл настройки dnsmasq (/etc/dnsmasq.conf) добавить ещё один интерфейс docker0 и отключить на нём "раздачу" динамических IP, ибо Docker сам присваивает контейнерам адреса. В файл конфигурации нужно добавить строки:
